...
plt_gif = Animation()
for iter = 1:storage_profit_function.fcalls
    # Final GP
    ...
    # Plot posterior.
    plt_range_aux = plot(collect(range_mul_factor) * 100 / max_load, p_curve,
        label="Range Evaluation",
        ylabel="Profit (USD)",
        xlabel="Bid Volume (% Market Share)",
    ...
    plot!(plt_range_aux, collect(range_mul_factor) * 100 / max_load, p_fx(collect(range_mul_factor)); 
        label="Posterior - " * string(RationalKernel)
    )
    scatter!(plt_range_aux, x[1:iter] * 100 / max_load, 
        y[1:iter]; label="Visited"
    )
